...
- **🔒 Enterprise Security:** Encrypts PII (Personally Identifiable Information) at rest using `AES-256-GCM` via `pgcrypto`. The scraper operates in a heavily restricted, non-root Docker jail with dropped capabilities.
...
1. **Ingestion API:** REST endpoint with strict DTO validation and source-level idempotency.
2. **Cleansing Worker:** Sanitization and formatting.
3. **Normalization Worker:** Parsing addresses, phones, and localization.
4. **Deduplication Worker:** Exact and fuzzy merging to Master Leads.
